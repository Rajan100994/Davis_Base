...
//                                                ps_col = tableFile.readShort();
                                 ...
                                 {
                                     System.out.println("A table cannot have multiple primary keys.....");
                                 }
                             }
                             else
                             {
...
//               System.out.println(insert_temp2.length());
            ...
            String col_vals[] = insert_temp2.split(",");

             ArrayList<String> col_vals1 =new ArrayList<>();

            for(int y=0;y<col_vals.length;y++)
            {
                col_vals1.add(col_vals[y]);
            }


            ArrayList<String> remaining_col=new ArrayList<>();
            Boolean length = true;
            int diff=col_names.length-col_vals.length;
            if(col_names.length < col_vals.length)
            {
                length = false;
            }
            else
...
//                
            ...
            //    System.out.println(flag+" "+length + " "+null_flag);
                ...
                if(flag==1 && length)
                {
                    ...
                    {
                         try {
                             RandomAccessFile clmns = new RandomAccessFile("Data/user_data/"+tableName+"/"+col_names[g]+".ndx","rw");
                             clmns.seek(0);
                             int data_type=clmns.read();
                             if(data_type==0x04)
                             {
                                 crecord_size+=1;
                             }
                             ...
                             else if(data_type==0x0C)
                             {
                                 ...
                                 crecord_size+=col_vals[g].length();
                             }
                         } catch (Exception ex) {
                             //Logger.getLogger(DavisBase.class.getName()).log(Level.SEVERE, null, ex);
                         }
                ...
                if(fileNames[i].isFile())
                {
                    ...
                    names_of_col.add(ndx_names.substring(0,ndx_names.indexOf(".")));
                     ...
                     ArrayList<String> ordered_val = new ArrayList<>();
                     int null_byte=0;
...
//                         }
//                     }   
                     ArrayList<String> ordered_val_type = new ArrayList<>();

                     for(int o=0;o<names_of_col.size()+15;o++){
                            ordered_val_type.add(".");
                        }
                     ...
                     int breq = 2+4+(2*names_of_col.size())+1+crecord_size+null_byte;
                      //  System.out.println(breq);
                       // System.out.println(names_of_col.size());
                        //String[] ordered_val= new String[100];

                        System.out.println("Size:"+ordered_val.size());

                        //ordered_val.add(0,"@");

                       // System.out.println("null byte"+null_byte);
                        //System.out.println("ord vals"+ordered_val);
                        for(int a=0;a<col_vals1.size();a++)
                        {
                         try {
                             RandomAccessFile cols_ord = new RandomAccessFile("Data/user_data/"+tableName+"/"+col_names1.get(a)+".ndx","rw");
                             cols_ord.seek(2);
                             int ord=cols_ord.read();
                             ordered_val.set(ord,col_vals[a]);
                             System.out.println(ordered_val);
                         } catch (Exception ex) {
                             //Logger.getLogger(DavisBase.class.getName()).log(Level.SEVERE, null, ex);
                         }
                        }
                        //for(int g=0;g<ordered_val.size();g++){
             //           System.out.println("help "+ordered_val);
                        //}
                 ...
                 //           System.out.println(psi);
                    ...
                    //        System.out.println("colnames"+names_of_col);
                            ...
                            for(int d=0;d<names_of_col.size();d++)
                            {
                              ...
                              cols_dt.seek(2);
                              int op=cols_dt.read();
                              ordered_val_type.set(op,Integer.toString(dt));
                              //insert_table.write(dt);
                            }
                            for(int o=0;o<ordered_val_type.size();o++)
                            {
                                if(ordered_val_type.get(o).equalsIgnoreCase("."))
                                {

                                }
                                else{
                                    //insert_table.writeByte(ordered_val_type.get(o).length());
                                    insert_table.writeByte(Integer.parseInt(ordered_val_type.get(o)));
                                }
                            }
                            for(int o=0;o<ordered_val.size();o++)
                            {
                                if(ordered_val.get(o).equalsIgnoreCase("."))
                                {

                                }
                                else{
                                    insert_table.writeByte(ordered_val.get(o).length());
                                    insert_table.writeBytes(ordered_val.get(o));
                                }
                            }
                         ...
                         try {
                             ...
                             rcols_records.writeBytes(ordered_val.get(ordp));
                             psx+=16;
                             rcols_records.seek(3);
                       ...
                       System.out.println("Null vals not allowed");    
                    }
                }
                else
                {
            ...
            {
                System.out.println("Please create the table "+tableName+" before inserting vals");
            }
...
public boolean check_datatype(String type)
{
    ...
    switch(type)
    {
        ...
        case "date":
                return true;
        default:
            throw new Exception("Your Datatype is invalid !!!");	
